...
class DrawBook(AVGApp):
  ...
  def __init__(self, width, height, folder):
    ...
    self.configuration = [[0,0], [0,0]]; # list containing the configuration of the scene (sublist for each row)
    ...
    for i in range(len(self.configuration)):
      ...
      for j in range(len(self.configuration[i])):
        ...
        if self.configuration[i][j] != 0 and os.path.isfile(path):
          ...
          Entry.Entry(path, str(self.configuration[i][j]), self.masterDivNode, x, y, self.width/5, self.height/5)
        else:
